...
/**
* \brief reserva de forma dinamica memoria para un dato tipo eArcade
* \param es void
			 ...
			 do
				 {
					 ...
					 switch (opcion)
					 {
						...
						case 2:
							...
							pedirText(bufferNomJuA,sizeof(bufferNomJuA),"Ingrese el Nombre del Juego","Nombre no valido",INTENTOS);
							strncpy(pArcade[posicion]->nombreJuego,bufferNomJuA,sizeof(bufferNomJuA));
							break;
						default:
							break;
					 }
...
int altaArcade(eArcade* pArcades[],eSalon* pSalones[],int lenA,int lenS)
{
	...
	if(pArcades!=NULL && lenA>0 && pSalones!=NULL && lenS>0)
	{
		...
		if(libre>=0)
			{
			eArcade* pArcade=nuevoArcadeM_D();
			if(pArcade!=NULL)
				{
				pArcades[libre]=pArcade;
					  ...
					  if(pedirInt(&bufferCantJugA,"Ingrese Cantidad de Jugadores","Ingrese Cantidad valido",MINIMO,MAXIMO10,INTENTOS)==0)
						  ...
						  if(pedirInt(&bufferCantMAxFA,"Ingrese Cantidad Maxima de fichas", "Ingrese tipo valido",MAXIMO10,MAXIMO500,INTENTOS)==0)
			    			 ...
			    			 if(pedirInt(&idSalon,"Ingrese Salon","Ingrese Salon valida",MINIMO,CAN_SALON,INTENTOS)==0 && buscarPosicionDeSalonporSuId(pSalones,lenS,idSalon)>=0)
								 {
								 arcade_setTipoSon(pArcades, &bufferTipoSonA);
								 arcade_setCantFichas(pArcades, &bufferCantMAxFA);
								 arcade_setCantJugadores(pArcades, &bufferCantJugA);
								 arcade_setNombreJuego(pArcades, bufferNomJuA);
								 arcade_setNacionalidadArcade(pArcades, bufferNacA);
								 pArcades->idSalon=idSalon;
								 pArcades->idArcade=dameUnIdNuevoA();
								 estado=0;
								 }else
									 puts("Id de Salon No valido");
						  	  }else
						  		  puts("Cantidad de Fichas no validas");
					  	 }else
							 puts("Cantidad de Jugadores no validos");
			    	 }else
						 puts("Nacionalidad incorrecto");
			  	  }else
					 puts("Nombred e Juego Incorrecto");
			  }else
				 puts("Tipo de Salon No valido");
	   }else
			 puts("No se ha podido procesar");
   return estado;
}
...
int eliminarArcadeDeLista(eArcade* pArcades[],int lenArcade)
{
	...
	if(pArcades!=NULL && lenArcade>0)
	{
		...
		if(auxPosArcade>=0 && pArcades[auxPosArcade]!=NULL)
		{
 ...
 */
 int bajaLogicaDeUnArcade(eArcade* pArcades[],int len,int idabajar)
 {
	...
	if(pArcades!=NULL && posicion>=0 && len>0)
	 {
		free(pArcades[posicion]);
		pArcades[posicion]=NULL;
		organizar_arrayA(pArcades, len, posicion);
		retorno=0;
	 }else
		 puts("No se ha podido procesar");
...
/**
* \brief Busca  un Arcade recibiendo como parámetro de búsqueda el Id de arcade.
...
int buscarPosicionDeArcadeporSuId(eArcade* pArcades[],int len,int id)
{
	int retorno=-1;
		if(pArcades!=NULL && len>0)
		{
			for (int i=0;i<len;++i)
			{
				if(pArcades[i]!=NULL && pArcades[i]->idArcade==id)
				{
					retorno=i;
					break;
				}
...
int ordenaArcadePorNombreJuego(eArcade* pArcades[], int len)
{
	...
	if(pArcades!=NULL && len>0)
	{
		do
		{
			...
			{
				if(pArcades[i]!=NULL && pArcades[i+1]!=NULL)
				{
	...
	if (pArcades!=NULL && len>0)
	{
		for (int i= 0;  i< len; i++)
		{
			if(pArcades[i]!=NULL)
			{
				...
				pArcades[i]->idArcade,pArcades[i]->nacionalidadArcade,pArcades[i]->nombreJuego,pArcades[i]->tipoSonidoArcade,
				pArcades[i]->cantidadJugadorArcade,pArcades[i]->capMaxFichaArcade,pArcades[i]->idSalon);
				contadordeCargados++;
			}
		}
		imprimirLinea();
...
int imprimirUnArcadeCargado(eArcade* pArcades[],int len,int idmostrar)
{
	...
	{
		printf(" %d %-30s %-30s %d\t%d\t  %d\t\t%d\n",
		pArcades[posicion]->idArcade,pArcades[posicion]->nacionalidadArcade,pArcades[posicion]->nombreJuego,pArcades[posicion]->tipoSonidoArcade,
		pArcades[posicion]->cantidadJugadorArcade,pArcades[posicion]->capMaxFichaArcade,pArcades[posicion]->idSalon);
		imprimirLinea();
		estado=0;
	}else
		puts("NO HAY DATOS CARGADOS");
 ...
 */
 int bajaDeSalonYsusArcadesDeLista(eSalon* pSalones[],int lenS,eArcade* pArcades[],int lenA)
 {
 	...
 	if(pSalones!=NULL && lenA>0)
 	{
 		...
 		if(auxPos>=0)
 		{
 			...
 			{
 				if(pArcades[i]!=NULL && pArcades[i]->idSalon==idSBaja)
 				{
 			...
 			puts("Debe ingresar un Id de Salon validor");
 		imprimirSalonCargado(pSalones, lenS);
 	}else
 		 puts("No se ha podido procesar ");
 	return retorno;
 ...
 /**
 * \brief setea tipo de sonido de arcde
...
int arcade_setTipoSon(eArcade* pArcade,int* tipoSonA)
 {
 	int retorno=-1;
 	if(pArcade!=NULL && tipoSonA!=NULL)
 		{
 		pArcade->tipoSonidoArcade=*tipoSonA;
 		retorno=0;
 		}
 	return retorno;
 }
 ...
 /**
  * \brief setea cantFicha de arcde
  * \param recibe como PUNTEROS A estructura y valor de tipo
  * \return Retorna 0 todo bien  y -1  si no
  */
int arcade_setCantFichas(eArcade* pArcade,int* cantFichA)
  {
  	int retorno=-1;
  	if(pArcade!=NULL && cantFichA!=NULL)
  		{
  		pArcade->capMaxFichaArcade=*cantFichA;
  		retorno=0;
  		}
  	return retorno;
  }
  ...
  /**
    * \brief setea cantdeJugadores de arcde
    * \param recibe como PUNTEROS A estructura y valor de tipo
    * \return Retorna 0 todo bien  y -1  si no
    */
int arcade_setCantJugadores(eArcade* pArcade,int* cantJugadoresA)
    {
    	int retorno=-1;
    	if(pArcade!=NULL && cantJugadoresA!=NULL)
    		{
    		pArcade->cantidadJugadorArcade=*cantJugadoresA;
    		retorno=0;
    		}
    	return retorno;
    }
 ...
 /**
 * \brief setea el nombre de juego en arcade
 * \param recibe como PUNTEROS A estructura y nombre de juego
 * \return Retorna 0 todo bien  y -1  si no
*/
 int arcade_setNombreJuego(eArcade* pArcade,char* nombreJuegoA)
 {
 	int retorno=-1;
 	if(pArcade!=NULL && nombreJuegoA!=NULL)
 		{
 		strncpy(pArcade->nombreJuego,nombreJuegoA,sizeof(pArcade->nombreJuego));
 		retorno=0;
 		}
 	return retorno;
 }
 ...
 /**
  * \brief setea la nacionalidad del  arcade
  * \param recibe como PUNTEROS A estructura y nombre de juego
  * \return Retorna 0 todo bien  y -1  si no
 */
  ...
  {
  	int retorno=-1;
  	if(pArcade!=NULL && nacionalidadA!=NULL)
  		{
  		strncpy(pArcade->nacionalidadArcade,nacionalidadA,sizeof(pArcade->nacionalidadArcade));
  		retorno=0;
  		}
  	return retorno;
  }
 ...
 /**
 * \brief organiza el array de arcades luego de eliminar un elemento
 ...
 */
 ...
 {
 	int retorno=-1;
 	if(pArcades!=NULL && len>0 && posicion>=0)
 	for (int i=posicion; i< len; ++i)
 		{
 		pArcades[posicion]=pArcades[posicion+1];
 		pArcades[posicion+1]=NULL;
 		retorno=0;
 		}
 	return retorno;
 }
 ...
 //===================================================================================
  ...
  {
 	 int posicion=buscarLugarLibreArrayA(pArcades, len);
 	 if(posicion>=0)
 	  	{
 		 eArcade* pArcade = nuevoArcadeM_D();
 		 pArcades[posicion]=pArcade;
