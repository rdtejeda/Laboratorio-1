...
*/
int loadAddEmployee(eGenerico *plistEmployee, int len)
{
	int retorno=-1;
	int id;
	char name[51];
	...
	if(buscarLugarArray(plistEmployee, TAMANO_ARRAY)>=0 && len>0)
		{
						...
						if(pedirText(name, sizeof(name), "Ingrese Nombre", "Ingrese Nombre valido", INTENTOS)==0)
						{
							if (pedirText(lastName, sizeof(lastName), "Ingrese Apellido", "Ingrese Apellido valido", INTENTOS)==0)
							{
								if(pedirFloat(&salary, "Ingrese salario", "Ingrese monto valido", SALMIN, SALMAX, INTENTOS)==0)
								{
									...
									{
										retornoAddEmplyee=agregarEarray(plistEmployee, TAMANO_ARRAY, id, name, lastName, salary, sector);
									}
								}
							}
						}
...
int loadRemoveEmployee(eGenerico *plistEmployee, int len)
{
	int retorno=-1;
	int idBaja;
	imprimirEarray(plistEmployee, TAMANO_ARRAY);
	pedirInt(&idBaja, "Ingrese el Id a dar de baja", "Ingrese ID valido", MINIMO, TAMANO_ARRAY, INTENTOS);
	eliminarLogIdArray(plistEmployee, TAMANO_ARRAY,idBaja);
	imprimirEarray(plistEmployee, TAMANO_ARRAY);
