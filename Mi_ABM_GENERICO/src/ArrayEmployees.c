 ...
 */
int iniciarEarray(eGenerico *plistEmployee,int len)
{
	int estado=-1;
 ...
 */
int agregarEarray(eGenerico *plistEmployee,int len,int id,char name[],char lastName[],float salary,int sector)
{
	int estado=-1;
	int posicionLibre;
	if(plistEmployee!=NULL && len>0 && id>0 && salary>=SALMIN && salary<=SALMAX && strlen(name)>0 && strlen(lastName)>0 && sector>=1 && sector<=5)
		{
			posicionLibre=buscarLugarArray(plistEmployee, TAMANO_ARRAY);
			if(posicionLibre>=0)
				{
...
*/
int buscarUnIdArray(eGenerico *plistEmployee,int len,int id)
{
	int retorno=-1;
		if(plistEmployee!=NULL && len>0)
		{
...
*/
int eliminarLogIdArray(eGenerico *plistEmployee,int len,int id)
{
	int retorno=-1;
	int posicion;
	if(plistEmployee!=NULL && len>0 && id>=MINIMO && id<=TAMANO_ARRAY)
		 {
			posicion=buscarUnIdArray(plistEmployee, len, id);
			if(posicion>=0)
			{
...
*/
int ordenarArray(eGenerico *plistEmployee,int len,int order)
{
	int banderaSwapp=-1;
	eGenerico auxiliar;
	int limite=len;
...
*/
int imprimirEarray(eGenerico *plistEmployee,int len)
{
		int estado=-1;
...
*/
 int modificarUnDatodelArray(eGenerico *plistEmployee, int len)
 {
	 int retorno=-1;
	 int idamodificar;
	 int posicion;
	 ...
	 if(plistEmployee!=NULL && len>0)
		 {
		 imprimirEarray(plistEmployee, TAMANO_ARRAY);
		 pedirInt(&idamodificar, "Ingrese Id a modificar", "Error Ingrese Id valido", MINIMO, TAMANO_ARRAY, INTENTOS);
		 if(idamodificar>0 && idamodificar<TAMANO_ARRAY)
		 {
			 posicion=buscarUnIdArray(plistEmployee, TAMANO_ARRAY, idamodificar);
		 }
		 if (posicion>=0)
			 {
			 do
 ...
 */
 int operateSalaryEmployees(eGenerico *plistEmployee,int len,float *totalSalary, float *promSalary, int *overProm)
 {
	 int retorno=-1;
	 float sumSalary=0;
	 int contSalary=0;
...
*/
int buscarLugarArray(eGenerico *plistEmployee,int len)
{
	int retorno=-1;
	if(plistEmployee!=NULL && len>0)
	{
 ...
 */
int largoArray(eGenerico *plistEmployee,int len)
{
	int retorno=-1;
	int contBusy=0;
	if(plistEmployee!=NULL && len>0)
